...
export const Home: React.FC<HomeProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="relative w-full bg-[#020617] text-slate-200 overflow-x-hidden font-sans selection:bg-gold-500/30">
      
      {/* --- HERO --- */}
      <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
           <img src="https://images.unsplash.com/photo-1462331940025-496dfbfc7564?q=80&w=2500" className="w-full h-full object-cover opacity-40 animate-ken-burns" alt="Cosmos" />
           <div className="absolute inset-0 bg-gradient-to-b from-[#020617] via-transparent to-[#020617]"></div>
        </div>
        <div className="relative z-10 text-center px-6 max-w-5xl space-y-6">
           <ScrollReveal>
             <div className="inline-flex items-center gap-3 px-6 py-2 rounded-md border border-white/10 bg-white/5 backdrop-blur-md mb-4 shadow-[0_0_20px_rgba(234,179,8,0.1)]">
                <span className="w-2 h-2 bg-gold-500 rounded-full animate-pulse"></span>
                <span className="text-[10px] font-black tracking-[0.4em] uppercase text-gold-400">Yohana 8:32</span>
             </div>
             <h1 className="text-6xl md:text-8xl lg:text-9xl font-black text-white tracking-tighter leading-[0.9] drop-shadow-2xl uppercase">GOD CARES <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold-300 via-gold-500 to-gold-300">365</span></h1>
             <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-12">
                <button onClick={() => onNavigate(StageId.BIBLE_STUDY)} className="group relative px-12 py-5 bg-white text-black font-black text-sm uppercase tracking-[0.2em] hover:bg-gold-400 hover:-translate-y-1 transition-all shadow-[0_20px_40px_rgba(0,0,0,0.4)] rounded-md overflow-hidden w-full sm:w-auto">
                ...
                <button onClick={() => onNavigate(StageId.MEDIA)} className="group relative px-12 py-5 border border-white/20 text-white font-black text-sm uppercase tracking-[0.2em] hover:bg-white/10 hover:-translate-y-1 transition-all rounded-md overflow-hidden w-full sm:w-auto">
      ...
      <section className="relative pt-24 pb-12">
        ...
        {[
          ...
          <div key={i} className="relative h-screen flex items-center overflow-hidden border-b border-white/5">
            <img src={step.img} className="absolute inset-0 w-full h-full object-cover opacity-20 transition-transform duration-[8s] ease-out hover:scale-110" alt="" />
            ...
            <div className="max-w-6xl mx-auto px-6 relative z-10 w-full">
              <ScrollReveal className={i % 2 === 1 ? "text-right ml-auto" : ""}>
                <div className={`inline-flex items-center gap-3 mb-6 ${i % 2 === 1 ? 'flex-row-reverse' : ''}`}>
                    <div className="w-12 h-12 bg-white/5 backdrop-blur-xl border border-white/10 rounded-full flex items-center justify-center text-gold-500 font-black text-xl">0{step.id}</div>
                    <span className="text-gold-500 font-black text-xs uppercase tracking-[0.4em]">{step.tag}</span>
                </div>
                <h2 className="text-4xl md:text-6xl font-serif text-white italic opacity-80 mb-6 leading-tight">{step.h2}</h2>
                <div className={`bg-white/5 p-10 md:p-14 border-l-4 ${i % 2 === 1 ? 'border-r-4 border-l-0 text-right' : 'border-l-4'} border-gold-500/50 backdrop-blur-md shadow-2xl max-w-3xl ${i % 2 === 1 ? 'ml-auto' : ''} rounded-sm transition-all hover:bg-white/[0.08]`}>
                  <h3 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tighter mb-6">{step.h3}</h3>
                  <div className="text-slate-300 font-serif italic text-lg md:text-xl leading-relaxed">{step.p}</div>
                </div>
              </ScrollReveal>
            </div>
          </div>
      ...
      <section className="relative pt-24 pb-12 bg-[#050b1d]">
         <div className="max-w-7xl mx-auto px-6 mb-16 text-center">
        ...
        {[
          ...
          <div key={i} className="relative h-screen flex items-center overflow-hidden border-b border-white/5">
            <img src={step.img} className="absolute inset-0 w-full h-full object-cover opacity-20" alt="" />
            <div className="absolute inset-0 bg-gradient-to-r from-[#050b1d] via-[#050b1d]/90 to-transparent"></div>
            <div className="max-w-6xl mx-auto px-6 relative z-10 w-full">
              <ScrollReveal className={i % 2 === 0 ? "text-right ml-auto" : ""}>
                <div className={`inline-flex items-center gap-3 mb-6 ${i % 2 === 0 ? 'flex-row-reverse' : ''}`}>
                    <div className="w-12 h-12 bg-red-500/10 backdrop-blur-xl border border-red-500/20 rounded-full flex items-center justify-center text-red-500 font-black text-xl">0{step.id}</div>
                    <span className="text-red-500 font-black text-xs uppercase tracking-[0.4em]">{step.tag}</span>
                </div>
                <h2 className="text-4xl md:text-6xl font-serif text-white italic opacity-80 mb-6 leading-tight">{step.h2}</h2>
                <div className={`bg-red-950/5 p-10 md:p-14 border-l-4 ${i % 2 === 0 ? 'border-r-4 border-l-0 text-right' : 'border-l-4'} border-red-600 backdrop-blur-md shadow-2xl max-w-3xl ${i % 2 === 0 ? 'ml-auto' : ''} rounded-sm transition-all hover:bg-red-950/10`}>
                  <h3 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tighter mb-6">{step.h3}</h3>
                  <div className="text-red-50/70 font-serif italic text-lg md:text-xl leading-relaxed">{step.p}</div>
                </div>
              </ScrollReveal>
            </div>
          </div>
      ...
      {/* --- Ukweli upo. --- */}
      <section className="relative pt-24 pb-12 bg-[#020617]">
         <div className="max-w-7xl mx-auto px-6 mb-16 text-center">
        ...
        {[
          ...
          <div key={i} className="relative h-screen flex items-center overflow-hidden border-b border-white/5">
            ...
            <div className="max-w-6xl mx-auto px-6 relative z-10 w-full">
              <ScrollReveal className={i % 2 === 1 ? "text-right ml-auto" : ""}>
                <div className={`inline-flex items-center gap-3 mb-6 ${i % 2 === 1 ? 'flex-row-reverse' : ''}`}>
                    <div className="w-12 h-12 bg-emerald-500/10 backdrop-blur-xl border border-emerald-500/20 rounded-full flex items-center justify-center text-emerald-500 font-black text-xl">0{step.id}</div>
                    <span className="text-emerald-500 font-black text-xs uppercase tracking-[0.4em]">{step.tag}</span>
                </div>
                <h2 className="text-4xl md:text-6xl font-serif text-white italic opacity-80 mb-6 leading-tight">{step.h2}</h2>
                <div className={`bg-emerald-950/5 p-10 md:p-14 border-l-4 ${i % 2 === 1 ? 'border-r-4 border-l-0 text-right' : 'border-l-4'} border-emerald-500/50 backdrop-blur-md shadow-2xl max-w-3xl ${i % 2 === 1 ? 'ml-auto' : ''} rounded-sm transition-all hover:bg-emerald-950/10`}>
                  <h3 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tighter mb-6">{step.h3}</h3>
                  <div className="text-slate-300 font-serif italic text-lg md:text-xl leading-relaxed">{step.p}</div>
                </div>
              </ScrollReveal>
      ...
      <section className="py-24 px-6 bg-[#020617]">
         <div className="max-w-7xl mx-auto space-y-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
               <ScrollReveal className="bg-white/[0.02] border border-white/5 p-12 rounded-xl shadow-inner group hover:bg-white/[0.04] transition-all">
                  <h3 className="text-4xl font-black text-white uppercase tracking-tight mb-8">Dilema ya Leo.</h3>
                  ...
                  <div className="space-y-6">
                     <div className="p-6 bg-white/5 border-l-2 border-gold-500 italic text-sm text-slate-300">
               ...
               <ScrollReveal className="bg-white/[0.02] border border-white/5 p-12 rounded-xl shadow-inner group hover:bg-white/[0.04] transition-all">
                  <h3 className="text-4xl font-black text-white uppercase tracking-tight mb-8">Ukweli Unapatikana Wapi?</h3>
                  <div className="p-12 bg-primary-950/20 border border-primary-500/20 rounded-lg text-center space-y-6 mb-10">
                     <div className="w-16 h-16 bg-primary-500/20 rounded-full flex items-center justify-center mx-auto text-primary-400 mb-4"><Search size={32}/></div>
                     <h5 className="text-2xl font-black text-white uppercase tracking-widest">KWENYE NENO LA MUNGU</h5>
                     <p className="text-primary-200 text-lg italic">"Uwatakase kwa ile kweli; neno lako ndiyo kweli" — <span className="text-gold-400 font-black">Yohana 17:17</span></p>
                  </div>
            ...
            <ScrollReveal className="max-w-4xl mx-auto space-y-6">
               ...
               <div className="space-y-3">
                  <div className={`rounded-lg border transition-all duration-500 overflow-hidden ${activeDef === 'ukweli' ? 'bg-white/5 border-gold-500/50 shadow-2xl' : 'bg-transparent border-white/5'}`}>
                     <button onClick={() => setActiveDef(activeDef === 'ukweli' ? null : 'ukweli')} className="w-full p-8 flex items-center justify-between text-left">
                        <h3 className={`text-xl font-black uppercase tracking-widest ${activeDef === 'ukweli' ? 'text-gold-500' : 'text-slate-400'}`}>Ukweli ni nini?</h3>
                        <ChevronDown className={`transition-transform duration-500 ${activeDef === 'ukweli' ? 'rotate-180 text-gold-500' : 'text-slate-600'}`} />
                     </button>
                     <div className={`px-10 overflow-hidden transition-all duration-500 ${activeDef === 'ukweli' ? 'max-h-96 pb-10 opacity-100' : 'max-h-0 opacity-0'}`}>
                        <p className="text-slate-300 text-lg leading-relaxed italic border-l-2 border-gold-500/30 pl-6 font-serif">
                  ...
                  <div className={`rounded-lg border transition-all duration-500 overflow-hidden ${activeDef === 'uongo' ? 'bg-white/5 border-red-500/50 shadow-2xl' : 'bg-transparent border-white/5'}`}>
                     <button onClick={() => setActiveDef(activeDef === 'uongo' ? null : 'uongo')} className="w-full p-8 flex items-center justify-between text-left">
                        <h3 className={`text-xl font-black uppercase tracking-widest ${activeDef === 'uongo' ? 'text-red-500' : 'text-slate-400'}`}>Uongo ni nini?</h3>
                        <ChevronDown className={`transition-transform duration-500 ${activeDef === 'uongo' ? 'rotate-180 text-red-500' : 'text-slate-600'}`} />
                     </button>
                     <div className={`px-10 overflow-hidden transition-all duration-500 ${activeDef === 'uongo' ? 'max-h-96 pb-10 opacity-100' : 'max-h-0 opacity-0'}`}>
                        <p className="text-slate-300 text-lg leading-relaxed italic border-l-2 border-red-500/30 pl-6 font-serif">
               ...
               <div className="flex flex-col sm:flex-row justify-center items-center gap-6 pt-12">
                  <button onClick={() => onNavigate(StageId.BIBLE_STUDY)} className="px-12 py-5 bg-gold-500 text-black font-black text-[10px] uppercase tracking-[0.4em] shadow-2xl hover:scale-105 transition-all">ANZA KUCHUNGUZA</button>
                  <button onClick={() => onNavigate(StageId.DECEPTION_VAULT)} className="px-12 py-5 border border-white/10 text-white font-black text-[10px] uppercase tracking-[0.4em] hover:bg-white/5 transition-all">ANGALIA KAMA UMEDANGANYWA</button>
               </div>
            </ScrollReveal>
            ...
            <div className="pt-24 space-y-16">
               <ScrollReveal className="text-center space-y-6">
                  <h2 className="text-3xl md:text-5xl font-medium text-gold-400 tracking-tight leading-none lowercase">
               ...
               <ScrollReveal className="max-w-4xl mx-auto space-y-4">
                  {QUESTIONS.map((item, idx) => (
                     <div key={idx} onClick={() => setActiveQuestion(activeQuestion === idx ? null : idx)} className={`group cursor-pointer rounded-xl transition-all duration-500 overflow-hidden border ${activeQuestion === idx ? 'bg-white/5 border-white/20 shadow-2xl scale-[1.02]' : 'bg-transparent border-white/5 hover:bg-white/[0.02]'}`}>
                        <div className="p-6 flex items-center justify-between">
                           <div className="flex items-center gap-4">
                              <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${activeQuestion === idx ? 'bg-gold-500 text-slate-950' : 'bg-white/5 text-slate-500'}`}>
                                 {item.icon}
                              </div>
                              <h3 className={`font-black text-base uppercase tracking-widest ${activeQuestion === idx ? 'text-white' : 'text-slate-400'}`}>
                                 {item.q}
                              </h3>
                           </div>
                           <ChevronDown size={18} className={`transition-transform duration-500 ${activeQuestion === idx ? 'rotate-180 text-gold-500' : 'text-slate-600'}`} />
                        </div>
                        <div className={`px-20 overflow-hidden transition-all duration-500 ease-in-out ${activeQuestion === idx ? 'max-h-40 pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                           <p className="text-slate-400 text-sm leading-relaxed font-light italic border-l-2 border-gold-500/20 pl-6">{item.a}</p>
                        </div>
                     </div>
                  ))}
               </ScrollReveal>
      ...
      <section className="py-24 px-6 bg-[#050b1d] border-t border-white/5">
               ...
               {TOOLS.map((tool, index) => (
                  ...
                  >
                    <div 
                      ...
                      className="group relative bg-white/[0.03] border border-white/5 rounded-2xl p-6 cursor-pointer hover:bg-white/[0.08] hover:border-gold-500/40 transition-all duration-500 flex flex-col items-center text-center space-y-4 shadow-sm"
                    >
                       <div className={`w-12 h-12 bg-black/40 border border-white/10 rounded-xl flex items-center justify-center ${tool.color} group-hover:bg-gold-500 group-hover:text-black transition-all duration-500 shadow-xl`}>
                          {tool.icon}
                       </div>
                       <div className="space-y-1">
                          <h3 className="text-xs md:text-sm font-black text-white uppercase tracking-wider group-hover:text-gold-400 transition-colors">{tool.title}</h3>
                          <p className="text-[10px] text-slate-500 font-light opacity-60 italic">{tool.desc}</p>
                       </div>
