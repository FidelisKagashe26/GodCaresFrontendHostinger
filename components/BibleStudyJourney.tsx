...
export const BibleStudyJourney: React.FC = () => {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-slate-950 border-r border-white/5">
      ...
      <div className="p-6 bg-gradient-to-br from-slate-900 to-black border-b border-white/5">
         <div className="flex items-center gap-4 mb-4">
            ...
            <div>
               <h3 className="text-white font-black uppercase text-sm tracking-tight">{userProfile?.name || 'Mwanafunzi'}</h3>
         ...
         <div className="bg-white/5 rounded-xl p-3 border border-white/10 space-y-2">
            <div className="flex justify-between items-center text-[9px] font-black uppercase text-slate-400 tracking-widest">
               ...
               <span className="text-white">{totalKP.toLocaleString()}</span>
            </div>
              ...
              return (
                ...
                >
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 transition-colors ${
                    isActive ? 'bg-gold-500 text-slate-950 shadow-lg' : isDone ? 'bg-green-500/20 text-green-500' : 'bg-white/5 text-slate-500'
                  }`}>
                    {isLocked ? <Lock size={16} /> : isDone ? <CheckCircle2 size={16} /> : m.icon}
                  </div>
                  
                  <div className="flex-1 min-w-0">
                    <h4 className={`text-xs font-bold truncate ${isActive ? 'text-gold-400' : 'text-slate-300'}`}>
                      {m.title}
                    </h4>
      ...
      <div className="p-4 bg-black/20 border-t border-white/5">
         <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-blue-900/20 to-transparent rounded-xl border border-blue-500/10">
            <div className="p-2 bg-blue-500/20 rounded-lg text-blue-400">
               <Trophy size={16} />
            </div>
            <div>
               <p className="text-[9px] font-black uppercase text-blue-300 tracking-widest">Motivational Tip</p>
               <p className="text-[10px] text-slate-400 italic leading-tight">"Ukweli huweka huru." Endelea kusoma!</p>
            </div>
         </div>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#020617] animate-fade-in flex flex-col h-screen overflow-hidden text-slate-300">
      {/* Mobile Header */}
      <header className="lg:hidden h-16 border-b border-white/5 bg-slate-950 px-4 flex items-center justify-between shrink-0 shadow-2xl z-[60]">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-gold-500 rounded-lg flex items-center justify-center text-primary-950">
            <GraduationCap size={16} />
          </div>
          <span className="text-xs font-black text-white uppercase tracking-widest">GC365 ACADEMY</span>
        </div>
        <button onClick={() => setIsSidebarOpen(true)} className="p-2 bg-white/5 rounded-lg text-gold-500">
           <Menu size={20} />
        </button>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {/* DESKTOP SIDEBAR */}
        <aside className="hidden lg:flex w-80 bg-slate-950 flex-col shrink-0 z-20">
          <SidebarContent />
        </aside>
        ...
        {isSidebarOpen && (
          <div className="lg:hidden fixed inset-0 z-[100] flex">
            ...
            <div className="relative w-80 max-w-[85vw] h-full bg-slate-950 shadow-2xl animate-slide-in">
              <button 
                onClick={() => setIsSidebarOpen(false)}
                className="absolute top-2 -right-12 p-2 text-white bg-white/10 rounded-full"
              >
                <X size={20} />
              </button>
              <SidebarContent />
