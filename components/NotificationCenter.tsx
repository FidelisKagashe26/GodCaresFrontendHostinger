...
export const NotificationCenter: React.FC<Props> = ({ 
  ...
  return (
    <>
      ...
      <div className="fixed top-24 right-4 md:right-8 w-80 md:w-96 bg-slate-900/95 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl z-[90] overflow-hidden flex flex-col animate-scale-in origin-top-right">
         {/* Header */}
         <div className="p-4 border-b border-white/10 flex justify-between items-center bg-black/40">
           <div className="flex items-center gap-2">
             <Bell size={16} className="text-gold-400" />
             <h3 className="font-bold text-white text-sm uppercase tracking-wider">Log Analytics</h3>
           ...
           <div className="flex gap-1">
              {notifications.length > 0 && (
                <>
                  <button onClick={onMarkAllRead} title="Mark all read" className="p-2 hover:bg-white/10 rounded-lg text-slate-400 hover:text-green-400 transition-colors"><Check size={14}/></button>
                  <button onClick={onClearAll} title="Clear all" className="p-2 hover:bg-white/10 rounded-lg text-slate-400 hover:text-red-400 transition-colors"><Trash2 size={14}/></button>
                </>
              )}
         ...
         <div className="max-h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
           {notifications.length === 0 ? (
             <div className="p-12 text-center text-slate-500 flex flex-col items-center">
               ...
               <p className="text-xs font-medium uppercase tracking-widest">No active logs</p>
             </div>
           ) : (
             <div className="divide-y divide-white/5">
               {notifications.map(n => (
                 <div 
                   key={n.id} 
                   /* Added type cast to handle overlap with custom 'event' type string used in App.tsx */
                   onClick={() => (n.type as string) === 'event' && onNavigateToEvent?.()}
                   className={`p-4 hover:bg-white/5 transition-colors relative group cursor-pointer ${n.read ? 'opacity-60' : 'bg-blue-500/5'}`}
                 >
                    <div className="flex gap-3">
                       ...
                       <div className="flex-1 space-y-1">
                          <div className="flex justify-between items-start">
                             ...
                             <button onClick={(e) => { e.stopPropagation(); onDismiss(n.id); }} className="text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"><X size={12} /></button>
                          </div>
